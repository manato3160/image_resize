...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            logger.info(f"リクエスト受信: path={self.path}, content_length={content_length}")
            ...
            logger.info(f"画像処理開始: mode={mode}, upscale_method={upscale_method}")
            ...
            try:
                ...
                self._send_error_response(400, f"画像データのデコードに失敗: {str(e)}")
                return
            
            logger.info(f"Base64デコード成功: image_size={len(image_data)} bytes")
            ...
            try:
                ...
                self._send_error_response(500, f"画像処理に失敗: {str(e)}")
                return
